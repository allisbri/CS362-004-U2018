	...
	for (n = 0; n < 2000; n++) {
		...
		for (j = 0; j < G.numPlayers; j++){
			if (j == p){
				if (c == 0){
					...
					exDeck[p] = G.deckCount[p] - closest;
				}
				else if (c > 1){
					exHand[p] = G.handCount[p] + 1; //accounting for discarded adventurer
					exDeck[p] = G.deckCount[p] - closest2;
				}
			}
			else{
		...
		for (j = 0; j < G.numPlayers; j++){
			if (exHand[j] == G.handCount[j]){
				...
				printf("failed test 1 for player %d with expected %d and actual %d and c %d c2 %d\n", j, exHand[j], G.handCount[j], c);
				testFailCount++;
			}
		}
		...
		for (j = 0; j < G.numPlayers; j++){
			if (exDeck[j] == G.deckCount[j]){
				...
				printf("failed test 2 for player %d with expected %d and actual %d and c %d\n", j, exDeck[j], G.deckCount[j], c);
				testFailCount++;
			}
		}
