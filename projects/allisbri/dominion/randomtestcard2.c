 /* Brian Allison
 *  CS362
 *  Description: This is a random tester for the village card
 * -----------------------------------------------------------------------
...
void testGreatHall(int *testPassCount, int *testFailCount, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus) {
  int r;
  r = cardEffect(village, choice1, choice2, choice3, state, handPos, bonus);
  if (r == 0){
	  printf("passed village call test\n");
	  testPassCount++;
  }
  else{
	  printf("failed village call test\n");
	  testFailCount++;
  }
}
...
int main() {
	...
	G.hand[p][randHandSelect] = village;
